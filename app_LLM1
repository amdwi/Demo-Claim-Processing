...
from groq import Groq  # <-- NEW: Real LLM Cloud Client Integration
...
class FNOLIntakeAgent:
    ...
    def process(self, raw_text: str):
        ...
        # Real Enterprise Pipeline Step 1: LLM text parsing
...
class DamageAssessmentAgent:
    ...
    def process(self, structured_claim: dict):
        ...
        # --- Real Enterprise Pipeline Step 2: VECTOR MATCH VEHICLE BRAND/MODEL ---
        ...
        # --- Real Enterprise Pipeline Step 3: VECTOR MATCH THE DAMAGED COMPONENTS ---
        total_estimate = 0
        total_labor = 0
        total_parts_cost = 0
        breakdown = []
...
st.title("🤖 Enterprise Agentic Claims Processing Platform (Groq + Vector DB)")
st.markdown("---")
...
# Check if API Key exists in Streamlit deployment secrets, otherwise fallback to user input field
    ...
    with col_right:
        ...
        if execute_pipeline:
            if not groq_api_key:
                ...
                # Pass the live API key directly into the Intake Agent
                ...
                p1 = st.status("🔄 [Groq LLM Engine] Extracting text entities natively...", expanded=True)
                claim_data = fnol_agent.process(final_processing_text)
                
                # Show the clean structural tokens extracted by the LLM
                if claim_data.get("policy_number") != "POL-ERROR":
                    st.markdown("##### 📦 Extracted Structural Tokens from LLM:")
                    st.json({
                ...
                time.sleep(0.2)
                p1.update(label=f"✅ Groq LLM Extraction Complete ({claim_data['claim_number']})", state="complete")
